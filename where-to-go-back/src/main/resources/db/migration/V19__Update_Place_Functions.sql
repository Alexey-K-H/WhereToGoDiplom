DROP FUNCTION IF EXISTS FILTER_CANDIDATES_BY_CATEGORY_IDS;
CREATE OR REPLACE FUNCTION FILTER_CANDIDATES_BY_CATEGORY_IDS(
                           ...
                           AS (SELECT V_PLACES_INFO.ID,
                               ...
                               WHERE V_PLACES_INFO.LATITUDE BETWEEN @MY_LAT - @DLAT
                                 ...
                                 AND V_PLACES_INFO.LONGITUDE BETWEEN @MY_LON - @DLON
                                   AND @MY_LON + @DLON
                                 AND V_PLACES_INFO.CATEGORY_ID = ANY (_IDS)
...
COMMENT ON FUNCTION FILTER_CANDIDATES_BY_CATEGORY_IDS(
    MY_LAT DOUBLE PRECISION,
    MY_LON DOUBLE PRECISION,
    DLAT DOUBLE PRECISION,
    DLON DOUBLE PRECISION,
    ...
    _MAX_DIST DOUBLE PRECISION,
    DEG2DIST DOUBLE PRECISION)
    IS 'Функция фильтрации множества найденных кандидатов с учетом категорий';
...
DROP FUNCTION IF EXISTS FIND_NEAREST_WITH_CATEGORY_IDS;
CREATE FUNCTION FIND_NEAREST_WITH_CATEGORY_IDS(
...
BEGIN
    ...
    IF (ABS(@MY_LON) + @DLON < 180 OR
        ...
        RETURN QUERY (SELECT *
                      FROM FILTER_CANDIDATES_BY_CATEGORY_IDS(
                              MY_LAT := MY_LAT,
                              MY_LON := MY_LON,
                              DLAT := DLAT,
                              DLON := DLON,
...
COMMENT ON FUNCTION FIND_NEAREST_WITH_CATEGORY_IDS(
    _MY_LAT DOUBLE PRECISION,
    _MY_LON DOUBLE PRECISION,
    _START_DIST DOUBLE PRECISION,
    _MAX_DIST DOUBLE PRECISION,
    _LIMIT INTEGER,
    _IDS TEXT)
    IS 'Основная функция поиска набора мест вблизи заданной геоточки с учетом категорий';
